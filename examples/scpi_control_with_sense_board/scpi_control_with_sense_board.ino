...
#define DESCRIPTION ("Power Board")
...
void loop() {
  ...
  //VAL is boolean 0 or 1
	...
	power_board.set_led(value_bool);
  }
  
  //INSTR:DAC:ADDR X ADDR  //X is placeholder for channel, ADDR is 7-bit right-justified address
  else if (CMDIS(command, "INSTR:DAC:ADDR")){
	power_board.set_dac_i2c_addr(uint8_t(value_int));
  }
  ...
  //VOLT VAL
	...
	val_float = String(str_float).toFloat();
	
	power_board.set_voltage(val_float);
  }
  ...
  //CURR VAL
	...
	power_board.set_current(val_float);
  }
  
  //PSU:ON
  else if (CMDIS(command, "PSU:ON")){
	power_board.enable_psu();
  }
  
  //PSU:OFF
  else if (CMDIS(command, "PSU:OFF")){
	power_board.disable_psu();
  }
  
  //ELOAD:ON
  else if (CMDIS(command, "ELOAD:ON")){
	power_board.enable_eload();
  }
  
  //ELOAD:OFF
  else if (CMDIS(command, "ELOAD:OFF")){
	power_board.disable_eload();
  }
  
  //POWER:CAL:RESET
  else if (CMDIS(command, "POWER:CAL:RESET")){
	power_board.reset_calibration();
  }
...
void parse_serial(char ser_buf[], char command[], char *token, uint8_t *channel_num, bool *value_bool, int16_t *value_int)
{
  ...
  else if (CMDIS(command, "SENSE:CAL:CURR")) //SENSE:CAL:CURR 123.123,123.123,123.123,123.123
  {
	  //now we want to parse 4 floats separated by commas
	  //do the rest of the parsing in the function above.
    ;
  }
  else if  (CMDIS(command, "CURR")) //CURR 123.123
  {
	  //do the rest of the parsing in the function above.
    ;
  }
  else if  (CMDIS(command, "VOLT")) //VOLT 123.123
  {
	  //do the rest of the parsing in the function above.
    ;
  }
